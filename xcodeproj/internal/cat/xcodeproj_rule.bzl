...
load("//xcodeproj/internal:xcschemes.bzl", "xcschemes_internal")
...
""")
    ...
    link_params = xctargets.create_link_params(
        actions = actions,
        generator_name = name,
    ...
    xcscheme_infos = xcschemes_internal.create_infos(
        ctx.attr.xcschemes_json,
        default_xcode_configuration = default_xcode_configuration,
        top_level_deps = top_level_deps,
    )

    (xcschemes, xcschememanagement) = pbxproj_partials.write_schemes(
        actions = actions,
    ...
    transitive_file_paths = [inputs.unsupported_extra_file_paths]
    transitive_folders = []
