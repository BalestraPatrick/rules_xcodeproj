...
def _collect_transitive_uncategorized_file_paths(info):
    if info.xcode_target:
        return EMPTY_DEPSET
    return info.inputs.uncategorized_file_paths
...
def _inner_merge_input_files(
    ...
    return struct(
        ...
        uncategorized_file_paths = memory_efficient_depset(
            transitive = [
                info.inputs.uncategorized_file_paths
                for info in transitive_infos
            ],
        ),
        ...
        unsupported_extra_file_paths = memory_efficient_depset(
            transitive = [
                info.inputs.unsupported_extra_file_paths
                for info in transitive_infos
            ],
        ),
...
def _process_cc_info_headers(headers, *, exclude_headers, generated):
    ...
    for header in headers:
        if header in exclude_headers:
            continue
        (file, file_path) = _process_header(header)
        if file:
            files.append(file)
        elif file_path:
...
def _process_files_and_deps(
        ...
        if file.is_source:
            if not categorized:
                if (file.basename == ".xccurrentversion" and
                    ...
                    (file, file_path) = normalized_file_path(
                        file,
                    ...
                    if file:
                        ...
                        uncategorized_file_paths.append(file_path)
        elif categorized:
            generated.append(file)

    transitive_extra_files = []
    transitive_extra_file_paths = []
    ...
    def _handle_dep(dep):
        ...
        transitive_extra_file_paths.append(
            dep[XcodeProjInfo].inputs.uncategorized_file_paths,
        )
    ...
    if build_mode == "xcode" and SwiftInfo in target:
    ...
    if CcInfo in target:
        ...
        (files, file_paths) = _process_cc_info_headers(
        ...
        extra_file_paths.extend(file_paths)
    ...
    if linker_input_additional_files:
        ...
        for file in linker_input_additional_files:
            ...
            if file:
                ...
                extra_file_paths.append(file_path)
    ...
    return (
        struct(
            ...
            extra_file_paths = memory_efficient_depset(
                extra_file_paths,
                transitive = transitive_extra_file_paths,
            ),
        ...
        struct(
            ...
            uncategorized_file_paths = memory_efficient_depset(
                uncategorized_file_paths,
                transitive = [
                    _collect_transitive_uncategorized_file_paths(info)
                    for info in transitive_infos
                ],
            ),
            uncategorized_files = memory_efficient_depset(
...
def _collect_unsupported_input_files(
    ...
    if is_resource_bundle:
        resource_bundle_uncategorized = uncategorized_file_paths
        uncategorized = None
    else:
    ...
    return struct(
        ...
        unsupported_extra_file_paths = memory_efficient_depset(
            extra_file_paths if include_extra_files else None,
            transitive = [
                info.inputs.unsupported_extra_file_paths
                for info in transitive_infos
            ] + (transitive_extra_file_paths if include_extra_files else []),
        ),
        unsupported_extra_files = memory_efficient_depset(
            extra_files,
            transitive = [
